...
<h4> Here you can see our newest dishes! </h4>
    </div>
    <div class="container" id="product-grid">
    	<div class="txt-heading">Products</div>
    	<?php
    	$product_array = $db_handle->runQuery("SELECT * FROM dishes ORDER BY idDishes ASC");
    	if (!empty($product_array)) {
    		foreach($product_array as $key=>$value){
    	?>
				<div class="media border p-3">
					<form method="post" action="Dishes.php?action=add&Code=<?php echo $product_array[$key]["Code"]; ?>">
						<img src="<?php echo $product_array[$key]["Image"]; ?>" class="mr-3 mt-3 rounded-circle" style="width:255px">
